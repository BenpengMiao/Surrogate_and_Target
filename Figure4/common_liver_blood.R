...
#3.  fold change of blood-liver common DARs
#female
data<-read.table("Li_Bl_adt_F_shared_DAR_logFC.txt",header = T)
pheatmap(data[,1:16],show_rownames = F,cluster_rows = F,cluster_cols = F, fontsize = 8,
         breaks = myBreaks,color=myColor)
#male
data<-read.table("Li_Bl_adt_M_shared_DAR_logFC.txt",header = T)
pheatmap(data[,1:16],show_rownames = F,cluster_rows = F,cluster_cols = F, fontsize = 8,
         breaks = myBreaks,color=myColor)

#4. fold change of blood-liver common DMRs
paletteLength <- 100
myColor <- colorRampPalette(c("blue", "white", "red"))(paletteLength)
myBreaks <- c(seq(-1, 0, length.out=ceiling(paletteLength/2) + 1),
              seq(1/paletteLength, 1, length.out=floor(paletteLength/2)))
#female
data<-read.table("overlap_blood_region_F_liver_F_logFC.txt",header = T)
pheatmap(data[,1:16],show_rownames = F,cluster_rows = F,cluster_cols = F, fontsize = 8,
         breaks = myBreaks,color=myColor,border_color = "white")
#male
data<-read.table("overlap_blood_region_M_liver_M_logFC.txt",header = T)
pheatmap(data[,1:16],show_rownames = F,cluster_rows = F,cluster_cols = F, fontsize = 8,
         breaks = myBreaks,color=myColor,border_color = "white")
